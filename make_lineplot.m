...
duration = 0.5; %duration for baseline;
base_pre = 2.5;
...
for a = 1:length(all_trials_valid),
   if all_trials_valid(a,25)>0;
       ...
       FR_1st(a,3) = (length(find(cell(:,1) <=(all_trials_valid(a,24)+3.0+post) & cell(:,1) >=(all_trials_valid(a,24)+3.0+pre)))...
           /((all_trials_valid(a,24)+3.0+post)-(all_trials_valid(a,24)+3.0+pre)));
...
%COMBINE ALL CELLS;
cat_bk1_sh(:,:,cell_count) = cat(3,cat(1,FR_bk1_sh_F,FR_bk1_sh_L));
cat_bk1_lo(:,:,cell_count) = cat(3,cat(1,FR_bk1_lo_F,FR_bk1_lo_L));

cat_bk2_sh(:,:,cell_count) = cat(3,cat(1,FR_bk2_sh_F,FR_bk2_sh_L));
cat_bk2_lo(:,:,cell_count) = cat(3,cat(1,FR_bk2_lo_F,FR_bk2_lo_L));

cat_bk3_sh(:,:,cell_count) = cat(3,cat(1,FR_bk3_sh_F,FR_bk3_sh_L));
cat_bk3_lo(:,:,cell_count) = cat(3,cat(1,FR_bk3_lo_F,FR_bk3_lo_L));

cat_bk4_sh(:,:,cell_count) = cat(3,cat(1,FR_bk4_sh_F,FR_bk4_sh_L));
cat_bk4_lo(:,:,cell_count) = cat(3,cat(1,FR_bk4_lo_F,FR_bk4_lo_L));

cat_bk5_sh(:,:,cell_count) = cat(3,cat(1,FR_bk5_sh_F,FR_bk5_sh_L));
cat_bk5_lo(:,:,cell_count) = cat(3,cat(1,FR_bk5_lo_F,FR_bk5_lo_L));
...
%Calcuate within-subject;
%calculate 'case mean" (average of all trials on each subject);
case_mean_bk1_sh = mean(cat_bk1_sh,1);
case_mean_bk1_lo = mean(cat_bk1_lo,1);
case_mean_bk2_sh = mean(cat_bk2_sh,1);
case_mean_bk2_lo = mean(cat_bk2_lo,1);
case_mean_bk3_sh = mean(cat_bk3_sh,1);
case_mean_bk3_lo = mean(cat_bk3_lo,1);
case_mean_bk4_sh = mean(cat_bk4_sh,1);
case_mean_bk4_lo = mean(cat_bk4_lo,1);
case_mean_bk5_sh = mean(cat_bk5_sh,1);
case_mean_bk5_lo = mean(cat_bk5_lo,1);
case_mean_cis_sh = mean(cat_cis_sh,1);
case_mean_cis_lo = mean(cat_cis_lo,1);
case_mean_tra_sh = mean(cat_tra_sh,1);
case_mean_tra_lo = mean(cat_tra_lo,1);

case_mean_sh = mean(cat_sh,1);
case_mean_lo = mean(cat_lo,1);

%calculate 'grand mean" (average of all 20 trials on all subjects);
grand_mean_bk1_sh = mean(mean(cat_bk1_sh,1),3);
grand_mean_bk1_lo = mean(mean(cat_bk1_lo,1),3);
grand_mean_bk2_sh = mean(mean(cat_bk2_sh,1),3);
grand_mean_bk2_lo = mean(mean(cat_bk2_lo,1),3);
grand_mean_bk3_sh = mean(mean(cat_bk3_sh,1),3);
grand_mean_bk3_lo = mean(mean(cat_bk3_lo,1),3);
grand_mean_bk4_sh = mean(mean(cat_bk4_sh,1),3);
grand_mean_bk4_lo = mean(mean(cat_bk4_lo,1),3);
grand_mean_bk5_sh = mean(mean(cat_bk5_sh,1),3);
grand_mean_bk5_lo = mean(mean(cat_bk5_lo,1),3);
grand_mean_cis_sh = mean(mean(cat_cis_sh,1),3);
grand_mean_cis_lo = mean(mean(cat_cis_lo,1),3);
grand_mean_tra_sh = mean(mean(cat_tra_sh,1),3);
grand_mean_tra_lo = mean(mean(cat_tra_lo,1),3);

grand_mean_sh = mean(mean(cat_sh,1),3);
grand_mean_lo = mean(mean(cat_lo,1),3);


%calcuate adjusted value 'grand mean - case mean';
for a = 1:cell_count;
    adj_value_bk1_sh(:,:,a) = grand_mean_bk1_sh - case_mean_bk1_sh(:,:,a);
    adj_value_bk1_lo(:,:,a) = grand_mean_bk1_lo - case_mean_bk1_lo(:,:,a);
    adj_value_bk2_sh(:,:,a) = grand_mean_bk2_sh - case_mean_bk2_sh(:,:,a);
    adj_value_bk2_lo(:,:,a) = grand_mean_bk2_lo - case_mean_bk2_lo(:,:,a);
    adj_value_bk3_sh(:,:,a) = grand_mean_bk3_sh - case_mean_bk3_sh(:,:,a);
    adj_value_bk3_lo(:,:,a) = grand_mean_bk3_lo - case_mean_bk3_lo(:,:,a);
    adj_value_bk4_sh(:,:,a) = grand_mean_bk4_sh - case_mean_bk4_sh(:,:,a);
    adj_value_bk4_lo(:,:,a) = grand_mean_bk4_lo - case_mean_bk4_lo(:,:,a);
    adj_value_bk5_sh(:,:,a) = grand_mean_bk5_sh - case_mean_bk5_sh(:,:,a);
    adj_value_bk5_lo(:,:,a) = grand_mean_bk5_lo - case_mean_bk5_lo(:,:,a);
    adj_value_cis_sh(:,:,a) = grand_mean_cis_sh - case_mean_cis_sh(:,:,a);
    adj_value_cis_lo(:,:,a) = grand_mean_cis_lo - case_mean_cis_lo(:,:,a);
    adj_value_tra_sh(:,:,a) = grand_mean_tra_sh - case_mean_tra_sh(:,:,a);
    adj_value_tra_lo(:,:,a) = grand_mean_tra_lo - case_mean_tra_lo(:,:,a);
    
    adj_value_sh(:,:,a) = grand_mean_sh - case_mean_sh(:,:,a);
    adj_value_lo(:,:,a) = grand_mean_lo - case_mean_lo(:,:,a);
end

%calculate adjusted numbers;
for a = 1:cell_count;
    for b = 1:First_T+Last_T;
        adj_bk1_sh(b,:,a) = cat_bk1_sh(b,:,a) + adj_value_bk1_sh(:,:,a);
        adj_bk1_lo(b,:,a) = cat_bk1_lo(b,:,a) + adj_value_bk1_lo(:,:,a);
        adj_bk2_sh(b,:,a) = cat_bk2_sh(b,:,a) + adj_value_bk2_sh(:,:,a);
        adj_bk2_lo(b,:,a) = cat_bk2_lo(b,:,a) + adj_value_bk2_lo(:,:,a);
        adj_bk3_sh(b,:,a) = cat_bk3_sh(b,:,a) + adj_value_bk3_sh(:,:,a);
        adj_bk3_lo(b,:,a) = cat_bk3_lo(b,:,a) + adj_value_bk3_lo(:,:,a);
        adj_bk4_sh(b,:,a) = cat_bk4_sh(b,:,a) + adj_value_bk4_sh(:,:,a);
        adj_bk4_lo(b,:,a) = cat_bk4_lo(b,:,a) + adj_value_bk4_lo(:,:,a);
        adj_bk5_sh(b,:,a) = cat_bk5_sh(b,:,a) + adj_value_bk5_sh(:,:,a);
        adj_bk5_lo(b,:,a) = cat_bk5_lo(b,:,a) + adj_value_bk5_lo(:,:,a);
        adj_cis_sh(b,:,a) = cat_cis_sh(b,:,a) + adj_value_cis_sh(:,:,a);
        adj_cis_lo(b,:,a) = cat_cis_lo(b,:,a) + adj_value_cis_lo(:,:,a);
        adj_tra_sh(b,:,a) = cat_tra_sh(b,:,a) + adj_value_tra_sh(:,:,a);
        adj_tra_lo(b,:,a) = cat_tra_lo(b,:,a) + adj_value_tra_lo(:,:,a);
        
        adj_sh(b,:,a) = cat_sh(b,:,a) + adj_value_sh(:,:,a);
        adj_lo(b,:,a) = cat_lo(b,:,a) + adj_value_lo(:,:,a);
    end
end

%Average and SEM;
avg_bk1_sh = mean(adj_bk1_sh,3);
avg_bk1_lo = mean(adj_bk1_lo,3);
avg_bk2_sh = mean(adj_bk2_sh,3);
avg_bk2_lo = mean(adj_bk2_lo,3);
avg_bk3_sh = mean(adj_bk3_sh,3);
avg_bk3_lo = mean(adj_bk3_lo,3);
avg_bk4_sh = mean(adj_bk4_sh,3);
avg_bk4_lo = mean(adj_bk4_lo,3);
avg_bk5_sh = mean(adj_bk5_sh,3);
avg_bk5_lo = mean(adj_bk5_lo,3);
avg_cis_sh = mean(adj_cis_sh,3);
avg_cis_lo = mean(adj_cis_lo,3);
avg_tra_sh = mean(adj_tra_sh,3);
avg_tra_lo = mean(adj_tra_lo,3);

avg_sh = mean(adj_sh,3);
avg_lo = mean(adj_lo,3);

sem_bk1_sh = std(adj_bk1_sh,0,3)/sqrt(cell_count);
sem_bk1_lo = std(adj_bk1_lo,0,3)/sqrt(cell_count);
sem_bk2_sh = std(adj_bk2_sh,0,3)/sqrt(cell_count);
sem_bk2_lo = std(adj_bk2_lo,0,3)/sqrt(cell_count);
sem_bk3_sh = std(adj_bk3_sh,0,3)/sqrt(cell_count);
sem_bk3_lo = std(adj_bk3_lo,0,3)/sqrt(cell_count);
sem_bk4_sh = std(adj_bk4_sh,0,3)/sqrt(cell_count);
sem_bk4_lo = std(adj_bk4_lo,0,3)/sqrt(cell_count);
sem_bk5_sh = std(adj_bk5_sh,0,3)/sqrt(cell_count);
sem_bk5_lo = std(adj_bk5_lo,0,3)/sqrt(cell_count);
sem_cis_sh = std(adj_cis_sh,0,3)/sqrt(cell_count);
sem_cis_lo = std(adj_cis_lo,0,3)/sqrt(cell_count);
sem_tra_sh = std(adj_tra_sh,0,3)/sqrt(cell_count);
sem_tra_lo = std(adj_tra_lo,0,3)/sqrt(cell_count);      

sem_sh = std(adj_sh,0,3)/sqrt(cell_count);
sem_lo = std(adj_lo,0,3)/sqrt(cell_count);      
...
%Figure 1; cis PE;
figure;
errorbar(x_F,avg_cis_sh(1:First_T,1),sem_cis_sh(1:First_T,1),'LineWidth',4,'Color',[0 0 1]);hold on; %blue, short reward;
errorbar(x_F,avg_cis_lo(1:First_T,2),sem_cis_lo(1:First_T,2),'LineWidth',4,'Color',[1 0 0]);hold on; %red, short omission;

errorbar(x_L,avg_cis_sh(First_T+1:First_T+Last_T,1),sem_cis_sh(First_T+1:First_T+Last_T,1),'LineWidth',4,'Color',[0 0 1]);hold on; %blue, short reward;
errorbar(x_L,avg_cis_lo(First_T+1:First_T+Last_T,2),sem_cis_lo(First_T+1:First_T+Last_T,2),'LineWidth',4,'Color',[1 0 0]);hold on; %red, short omission;

title('cis short reward and omission');
box off;

%Figure 2; trans PE;
figure;
errorbar(x_F,avg_tra_sh(1:First_T,1),sem_tra_sh(1:First_T,1),'LineWidth',4,'Color',[0 .8 1]);hold on; %blue, short reward;
errorbar(x_F,avg_tra_lo(1:First_T,2),sem_tra_lo(1:First_T,2),'LineWidth',4,'Color',[1 .2 .4]);hold on; %red, short omission;

errorbar(x_L,avg_tra_sh(First_T+1:First_T+Last_T,1),sem_tra_sh(First_T+1:First_T+Last_T,1),'LineWidth',4,'Color',[0 .8 1]);hold on; %blue, short reward;
errorbar(x_L,avg_tra_lo(First_T+1:First_T+Last_T,2),sem_tra_lo(First_T+1:First_T+Last_T,2),'LineWidth',4,'Color',[1 .2 .4]);hold on; %red, short omission;

title('trans short reward and omission');
box off;

%Figure 3; long omission;
figure;
errorbar(x_F,avg_cis_sh(1:First_T,3),sem_cis_sh(1:First_T,3),'LineWidth',4,'Color',[0 0.498 0]);hold on; %blue, cis long omission;
errorbar(x_F,avg_tra_sh(1:First_T,3),sem_tra_sh(1:First_T,3),'LineWidth',4,'Color',[0 1 0]);hold on; %red, trans long omission;

errorbar(x_L,avg_cis_sh(First_T+1:First_T+Last_T,3),sem_cis_sh(First_T+1:First_T+Last_T,3),'LineWidth',4,'Color',[0 0.498 0]);hold on; %blue, cis long omission;
errorbar(x_L,avg_tra_sh(First_T+1:First_T+Last_T,3),sem_tra_sh(First_T+1:First_T+Last_T,3),'LineWidth',4,'Color',[0 1 0]);hold on; %red, trans long omission;

title('cis vs trans long omission');
box off;
